...
type HumanSpecs struct {
	Speed int
	Burst int
	Rest  int
}

func DistanceTraveled(specs *HumanSpecs, duration int) int {
	full_cycles := (duration / (specs.Burst + specs.Rest)) * specs.Speed * specs.Burst

	reminder := duration % (specs.Burst + specs.Rest)
	if reminder > specs.Burst {
		return full_cycles + specs.Speed*specs.Burst
	}
	return full_cycles + specs.Speed*reminder
}

func FarthestDistance(specs []*HumanSpecs, duration int) int {
	var farthest int = -1
	for _, spec := range specs {
		distance := DistanceTraveled(spec, duration)
...
Did I mention that these people are weird? They can only be running (always at their top speed), or resting (not moving at all), and can only spend integer amounts of time in either state.
...
At the 100th second, John is resting after running 240m while James has traveled 200m. In this case, John would have won if the race ended at 100 seconds.
